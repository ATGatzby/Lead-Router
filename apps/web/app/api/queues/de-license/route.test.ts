...
function makeRequest(body: unknown) {
  return new Request("http://localhost/api/queues/de-license", {
    method: "POST",
...
describe("POST /api/queues/de-license", () => {
  ...
  it("returns 404 when some queues are not found in org", async () => {
    ...
    const res = await POST(makeRequest({ queueIds: ["q1", "q999"] }));
    expect(res.status).toBe(404);
    const body = await res.json();
    expect(body.error).toMatch(/One or more queues not found/);
  });

  it("returns affected: 0 when all queues are already de-licensed", async () => {
    mockPrisma.sfdcQueue.findMany.mockResolvedValue([
      { id: "q1", name: "Sales Queue", isLicensed: false },
      { id: "q2", name: "Support Queue", isLicensed: false },
    ]);
  ...
  it("de-licenses queues without affecting user seats", async () => {
    mockPrisma.sfdcQueue.findMany.mockResolvedValue([
      { id: "q1", name: "Sales Queue", isLicensed: true },
      { id: "q2", name: "Support Queue", isLicensed: false },
      { id: "q3", name: "Ops Queue", isLicensed: true },
    ]);

...
} from "@/lib/ai-team-prompt";
import type { AvailableUser } from "@/lib/ai-team-prompt";
  ...
  try {
    ...
    if (users.length === 0) {
      return NextResponse.json(
        {
          error:
            "No licensed users found. Go to Settings \u2192 Users and license at least one user first.",
        },
        { status: 400 },
      );
    }
    ...
    // ── Build system prompt ──────────────────────────────────────────────
    const systemPrompt = buildTeamSystemPrompt(users);
    ...
    try {
      ...
      console.error(
        "[AI Team] Failed to parse AI response as JSON:",
        text.slice(0, 500),
      );
      return NextResponse.json(
        {
    ...
    let validated;
    try {
      validated = validateTeamResponse(parsed, users);
    } catch (err) {
      console.error("[AI Team] Validation error:", err);
      return NextResponse.json(
        {
          error:

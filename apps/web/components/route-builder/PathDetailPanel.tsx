...
function ConditionToken({ condition }: { condition: Condition }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-xs">
      <span className="font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 px-1.5 py-0.5 rounded">
        {condition.fieldApiName}
      </span>
      <span className="text-muted-foreground">{opLabel}</span>
      {!NO_VALUE_OPS.has(condition.operator) && condition.value && (
        <span className="font-medium text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950 px-1.5 py-0.5 rounded">
          {condition.value}
        </span>
      )}
    </span>
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-4">
              ...
              {assignmentLabel ? (
                <div className="rounded-md border bg-muted/30 p-2.5 space-y-1.5">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="font-medium text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950 px-1.5 py-0.5 rounded">
                      {assignmentLabel}
                    </span>
                  </div>
        ...
        {/* Search Trigger section detail */}
        {section.type === "trigger" && section.id === "search-trigger" && searchTrigger && (
          <>
            <div>
              <h4 className="text-[11px] font-semibold uppercase text-muted-foreground mb-2">
                Configuration
              ...
              <div className="rounded-md border bg-muted/30 p-2.5 space-y-1.5">
                ...
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="font-medium">
                    {searchTrigger.frequency ? `${searchTrigger.frequency.charAt(0) + searchTrigger.frequency.slice(1).toLowerCase()} at ${searchTrigger.scheduleTime} ${searchTrigger.scheduleTimezone}` : "One-time (manual)"}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-muted-foreground">Batch size:</span>
                  <span className="font-medium">{searchTrigger.batchSize} records</span>
                </div>
                {searchTrigger.triggerName && (
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-muted-foreground">Name:</span>
                    <span className="font-medium">{searchTrigger.triggerName}</span>
                  </div>
                )}
                {searchTrigger.skipRecentlyRouted && (
                  <div className="flex items-center gap-2 text-xs">
                    <span className="font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950 px-1.5 py-0.5 rounded">
                      Skip recently routed
                    </span>
                  </div>
                )}
                {searchTrigger.isDryRun && (
                  <div className="flex items-center gap-2 text-xs">
                    <span className="font-medium text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950 px-1.5 py-0.5 rounded">
                      Dry Run Active
                    </span>
          ...
          <>
            <div>
              ...
              <div className="rounded-md border bg-muted/30 p-2.5 space-y-1.5">
                {matchConfig.checkLeads && (
                  <div className="flex items-center gap-2 text-xs">
                    <span className="font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950 px-1.5 py-0.5 rounded">Leads</span>
                ...
                {matchConfig.checkContacts && (
                  <div className="flex items-center gap-2 text-xs">
                    <span className="font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950 px-1.5 py-0.5 rounded">Contacts</span>
                ...
                {matchConfig.checkAccounts && (
                  <div className="flex items-center gap-2 text-xs">
                    <span className="font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950 px-1.5 py-0.5 rounded">Accounts</span>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {matchConfig.matchEmail && <span className="text-xs font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 px-1.5 py-0.5 rounded">Email</span>}
                {matchConfig.matchPhone && <span className="text-xs font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 px-1.5 py-0.5 rounded">Phone</span>}
                {matchConfig.matchDomain && <span className="text-xs font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 px-1.5 py-0.5 rounded">Domain</span>}
                {matchConfig.matchCompanyName && <span className="text-xs font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 px-1.5 py-0.5 rounded">Company Name</span>}
              </div>
            </div>

...
export function MatchStepCard({ matchConfig, stepNumber, onEdit, onRemove }: Props) {
  ...
  return (
    <div
      className={cn(
        "relative rounded-xl border-2 border-blue-200 bg-blue-50 px-5 py-4",
        "flex items-center justify-between gap-4 w-full max-w-lg"
      )}
    >
      {/* Left: icon + summary */}
      <div className="flex items-center gap-3 min-w-0">
        <div className="flex-shrink-0 flex items-center justify-center size-9 rounded-lg bg-blue-100 text-blue-600">
        ...
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            ...
            {checkingItems.length === 0 && matchOnItems.length === 0 && (
              <span className="text-xs text-blue-400 italic">Not configured</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          className="text-blue-300 hover:text-destructive hover:bg-destructive/10"
          onClick={onRemove}

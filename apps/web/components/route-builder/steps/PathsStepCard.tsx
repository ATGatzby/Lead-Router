      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {paths.map((path, idx) => (
          <div
            key={path.id}
            className={cn(
              "flex-shrink-0 w-64 rounded-xl border-2 border-slate-200 bg-slate-50",
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2 px-3 py-2.5 border-b border-slate-200 bg-white">
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <span className="flex-shrink-0 flex items-center justify-center size-5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold">
                  {idx + 1}
                </span>
        ...
        <div className="flex-shrink-0 flex items-start pt-1">
          <button
            ...
            onClick={onAddPath}
            className={cn(
              "group flex items-center gap-2 rounded-xl border-2 border-dashed border-slate-200",
              "px-4 py-3 text-sm text-slate-400 bg-transparent",

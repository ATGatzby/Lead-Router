...
export class SalesforceApi {
  ...
  /**
   * Execute multiple REST API subrequests in a single round-trip.
   ...
   */
  async composite(subrequests: CompositeSubrequest[]): Promise<CompositeResult[]> {
    ...
    if (!res.ok) {
      const body = await res.text()
      throw new Error(`Composite request failed (${res.status}): ${body}`)
    }
    const data = (await res.json()) as { compositeResponse: CompositeResult[] }

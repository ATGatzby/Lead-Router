    ...
    for (const [dirName, metaType] of Object.entries(META_TYPE_MAP)) {
      ...
      for (const entry of entries) {
        if (dirName === 'lwc' && entry.isDirectory()) {
          ...
          // Track member name from any file (source or -meta.xml)
          // For types with only -meta.xml (PermissionSet, NamedCredential, etc.),
          // the member name is the first segment before the first dot.
          const memberName = entry.name.split('.')[0]
